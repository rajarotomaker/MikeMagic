...
class VideoInferencePipeline:
    ...
    def run(self, cond_frames, mask_frames, seed=42, mask_cond_mode="vae", fps=7, motion_bucket_id=127,
        ...
        with torch.no_grad():
            ...
            added_time_ids = self._get_add_time_ids(fps - 1, motion_bucket_id, noise_aug_strength, batch_size=1)
            ...
            video_tensor = (video_tensor / 2.0 + 0.5).clamp(0, 1)
            video_tensor = video_tensor.mean(dim=1, keepdim=True).repeat(1, 3, 1, 1)

            # Return a list of PIL images (cast to float32 as ToPILImage doesn't support BFloat16)
            return [transforms.ToPILImage()(frame.float()) for frame in video_tensor]
